...
int initWindow()
{
	...
	/* FIX: Set font NHO truoc khi set buffer/check max window.
	   Bug goc: check dwMaximumWindowSize truoc khi doi font -> font lon
	   default lam max rows < SCREEN_HEIGHT -> crash. */
	...
	/* Lay lai max window sau khi doi font */
